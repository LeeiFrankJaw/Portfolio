...
(define a 2)
(define cached1 (cached-assoc '((1 . 2) (3 . 4) (5 . 6) (7 . 8)) 3))
...
(define tests
  (test-suite
   ...
   (check-equal? (string-append-map
                  (list "Frank" "Lei" "Dora" "Linlin") ", hello.")
                 '("Frank, hello." "Lei, hello." "Dora, hello." "Linlin, hello."))
   
   ; list-nth-mod test
   (check-equal? (list-nth-mod (list 0 1 2 3 4) 2) 2 "list-nth-mod test")
   (check-equal? (list-nth-mod (sequence 0 49 1) 54) 4)
   ...
   (check-equal? (stream-for-n-steps (cycle-lists (list 1 2 3) (list "a" "b")) 3) (list (cons 1 "a") (cons 2 "b") (cons 3 "a")) 
                 "cycle-lists test")
   (check-equal? (stream-for-n-steps (cycle-lists (sequence 1 50 1) (sequence 2 100 2)) 3)
                 '((1 . 2) (2 . 4) (3 . 6)))
   (check-equal? (list-nth-mod (stream-for-n-steps (cycle-lists (sequence 1 50 1) (sequence 2 100 2)) 100) 99)
                 '(50 . 100))

   ; vector-assoc test
   (check-equal? (vector-assoc 4 (vector (cons 2 1) (cons 3 1) (cons 4 1) (cons 5 1))) (cons 4 1) "vector-assoc test")
   (check-equal? (vector-assoc 3 #((1 . 2) "hello" (3 . 4))) '(3 . 4))
   (check-equal? (vector-assoc 5 #((1 . 2) "hello" (3 . 4))) #f)

   ; cached-assoc tests
   (check-equal? ((cached-assoc (list (cons 1 2) (cons 3 4)) 3) 3) (cons 3 4) "cached-assoc test")
   (check-equal? (cached1 1) '(1 . 2))
   (check-equal? (cached1 1) '(1 . 2))
   (check-equal? (cached1 2) #f)
   (check-equal? (cached1 3) '(3 . 4))
   (check-equal? (cached1 3) '(3 . 4))
   (check-equal? (cached1 4) #f)
   (check-equal? (cached1 5) '(5 . 6))
   (check-equal? (cached1 5) '(5 . 6))
   (check-equal? (cached1 6) #f)
   (check-equal? (cached1 7) '(7 . 8))
   (check-equal? (cached1 7) '(7 . 8))
   (check-equal? (cached1 1) '(1 . 2))
