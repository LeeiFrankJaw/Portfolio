...
class Family(object):
    ...
    def cousin(self, a, b):
        ...
        a_parents = back_track(a_node)
        b_parents = back_track(b_node)
        if len(a_parents) > len(b_parents):
            a_parents, b_parents = b_parents, a_parents
        while len(a_parents) != 0 and a_parents[-1] == b_parents[-1]:
            a_parents.pop()
            b_parents.pop()
        return len(a_parents)-1, len(b_parents)-len(a_parents)
