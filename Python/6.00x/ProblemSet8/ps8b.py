...
##random.seed(3)
##p = Patient([SimpleVirus(0.5, 0.2)], 100)
##print p.getViruses()[0].getMaxBirthProb()
##print p.getViruses()[0].getClearProb()
##print p.getMaxPop()
##print p.getTotalPop()
##for i in range(10):
##    print p.update()
...
def simulationWithoutDrug(numViruses, maxPop, maxBirthProb, clearProb,
    ...
    for e in range(numTrials):
        p = Patient([SimpleVirus(maxBirthProb, clearProb)
                     for e in range(numViruses)], maxPop)
##        print p.getTotalPop()
        for i in range(300):
            avgViruses[i] += p.update()
##        print avgViruses
    pylab.plot([e/float(numTrials) for e in avgViruses],
               label='Average Viruses Population')
    pylab.title('Simulation without drug in %s trials' % numTrials)
    pylab.xlabel('Time step')
    pylab.ylabel('Average Viruses Population')
##    ymin, ymax = pylab.ylim()
##    pylab.text(210, 0.5*ymin+0.5*ymax, 'maxBirthProb: %s\nclearProb: %s\nmaxPop: %s\nnumViruses: %s'
##               % (maxBirthProb, clearProb, maxPop, numViruses),
##               verticalalignment='center')
    pylab.legend()
    pylab.show()

##simulationWithoutDrug(100, 1000, 0.1, 0.1, 100)
...
class ResistantVirus(SimpleVirus):
    ...
    def isResistantTo(self, drug):
        ...
        # TODO
        return self.resistances.has_key(drug) and self.resistances[drug]
...
class TreatedPatient(Patient):
    ...
    def addPrescription(self, newDrug):
        ...
        # TODO
        if not self.drugs.count(newDrug):
    ...
    def update(self):
        ...
        for i in self.viruses:
            try:
                newViruses.append(i.reproduce(popDensity, self.drugs))
            except NoChildException:
                pass
        self.viruses += newViruses
        return len(self.viruses)
...
##random.seed(2)
##p = TreatedPatient([ResistantVirus(0.5, 0.2, {'a':False}, 0.0)], 100)
##print p.getViruses()[0].getMaxBirthProb()
##print p.getViruses()[0].getClearProb()
##print p.getResistPop(['a'])
##print p.getMaxPop()
##print p.getTotalPop()
##p.addPrescription('a')
...
def simulationWithDrug(numViruses, maxPop, maxBirthProb, clearProb, resistances,
    ...
    # TODO
    avgViruses, avgGrViruses = [[0]*300 for e in range(2)]
##    print avgViruses, avgGrViruses
    ...
    pylab.plot([e/float(numTrials) for e in avgViruses],
               label='virus')
    pylab.plot([e/float(numTrials) for e in avgGrViruses],
               label='GR virus')
