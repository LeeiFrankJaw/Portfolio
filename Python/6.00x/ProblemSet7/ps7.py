...
class RectangularRoom(object):
    ...
    def __init__(self, width, height):
        ...
        height: an integer > 0
        """
        self.cleaned = [[False]*height for e in range(width)]
        self.height = height
        self.width = width
    ...
    def cleanTileAtPosition(self, pos):
        ...
        pos: a Position
        """
        if not (0 <= pos.getX() < self.width and
                0 <= pos.getY() < self.height):
            raise ValueError('The robot is moving out of room')
        self.cleaned[int(math.floor(pos.getX()))][int(math.floor(pos.getY()))]\
                                                            = True
    ...
    def isTileCleaned(self, m, n):
        ...
        """
        if not (0 <= m < self.width and 0 <= n < self.height):
            raise ValueError
    ...
    def getNumCleanedTiles(self):
        ...
        returns: an integer
        """
        return sum([e.count(True) for e in self.cleaned])
...
##    anim = ps7_visualize.RobotVisualization(num_robots, width, height)
    while num_trials > 0:
        ...
        robots = [robot_type(room, speed) for e in range(num_robots)]
##        anim.update(room, robots)
        while (float(room.getNumCleanedTiles()) /
...
##        anim.done()
        num_trials -= 1
    return float(totalTime) / totalTrials
...
##showPlot1('Effects of Number of Robots', 'Number of Robots', 'Time Steps')
...
##showPlot2('Effects of Aspect Ratio', 'Aspect Ratio', 'Time Steps')
