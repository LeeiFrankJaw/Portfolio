...
def equal(u,v):
    ...
    """
    assert u.D == v.D
    return all([ u[d]==v[d] for d in u.f.keys() | v.f.keys() ])
##    return all(map(lambda d: u[d]==v[d], u.D))
...
def scalar_mul(v, alpha):
    ...
    True
    """
    return Vec(v.D, { d:alpha*v[d] for d in v.f })
...
def neg(v):
    ...
    True
    """
    return Vec(v.D, { d:-v[d] for d in v.f })
