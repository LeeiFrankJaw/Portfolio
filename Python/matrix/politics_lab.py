...
def create_voting_dict(strlist):
    ...
    return { k:[int(x) for x in v]
             for (k,_,_,*v) in [s.split() for s in strlist] }
...
def most_similar(sen, voting_dict):
    ...
    return max([ (policy_compare(sen, k, voting_dict), k)
                 for k in voting_dict if k!=sen ])[1]
...
def least_similar(sen, voting_dict):
    ...
    Example:
        ...
        'c'
    """
    return min([ (policy_compare(sen, k, voting_dict), k)
                 for k in voting_dict if k!=sen ])[1]
...
def find_average_similarity(sen, sen_set, voting_dict):
    ...
    Example:
        ...
        -0.5
    """
    return sum([ policy_compare(sen, k, voting_dict)
                 for k in sen_set ])/len(sen_set)
...
def find_average_record(sen_set, voting_dict):
    ...
    Example: 
        ...
        [0.0, 1.0, 1.0]
    """
    return [ sum(xs)/len(sen_set)
             for xs in zip(*map(lambda k: voting_dict[k], sen_set)) ]
...
def bitter_rivals(voting_dict):
    ...
    n = len(sens)
    return min([ (policy_compare(sens[i], sens[j], voting_dict),
                  (sens[i], sens[j]))
