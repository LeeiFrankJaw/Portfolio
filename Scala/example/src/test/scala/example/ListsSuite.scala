...
class ListsSuite extends FunSuite {
  ...
  test("sum of a list of repeated elements") {
    assert(sum(List(9, 9, 9, 9, 9)) === 45)
  }
  
  test("sum of miscellaneous") {
    assert(sum(List(-5, 0, 7, 10, -11)) === 1)
  }
  ...
  test("max of a empty list throws an exception") {
    intercept[java.util.NoSuchElementException] {
      max(List())
    }
  }
  
  test("max of a list of repeated elements") {
    assert(max(List(9, 9, 9, 9, 9)) === 9)
  }
  
  test("max of miscellaneous") {
    assert(max(List(-5, 0, 7, 10, -11)) === 10)
  }
